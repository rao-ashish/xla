...
class Client : public llvm::RTTIExtends<Client, llvm::RTTIRoot> {
 public:
  ...
  // Donates each dst_array to the returned output (the output arrays take
  // ownership of the underlying device memory).
  //
  ...
  virtual absl::StatusOr<std::vector<ArrayRef>> CopyArraysInto(
      absl::Span<ArrayRef> src_arrays, absl::Span<ArrayRef> dst_arrays,
      ArrayCopySemantics semantics) {
    return absl::UnimplementedError(
        "This IFRT client does not support CopyArraysInto().");
  }
